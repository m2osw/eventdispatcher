 ...
 * you may want to consider using the local_stream_client_buffer_connection
 ...
 */
bool local_stream_server_client_buffer_connection::has_input() const
{
    return !f_line.empty();
}
 ...
 */
bool local_stream_server_client_buffer_connection::has_output() const
{
 ...
 */
bool local_stream_server_client_buffer_connection::is_writer() const
{
 ...
 *
 * \return The number of bytes written or -1 if the connection is closed.
 */
ssize_t local_stream_server_client_buffer_connection::write(void const * data, size_t const length)
{
    if(get_socket() == -1)
    {
        errno = EBADF;
 ...
 * process_read() of the local_stream_client_buffer_connection and
 * the pipe_buffer_connection classes.
 */
void local_stream_server_client_buffer_connection::process_read()
{
    ...
    local_stream_server_client_connection::process_read();
}
 ...
 */
void local_stream_server_client_buffer_connection::process_write()
{
    if(get_socket() != -1)
    {
        errno = 0;
        ssize_t const r(local_stream_server_client_connection::write(&f_output[f_position], f_output.size() - f_position));
        if(r > 0)
        {
    ...
    // process next level too
    local_stream_server_client_connection::process_write();
}
 ...
 */
void local_stream_server_client_buffer_connection::process_hup()
{
    // this connection is dead...
    //
    close();

    local_stream_server_client_connection::process_hup();
}
