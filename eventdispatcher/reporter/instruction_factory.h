...
{
public:
                            instruction_factory(instruction::pointer_t inst);
    virtual                 ~instruction_factory();

    std::string const &     get_name() const;

private:
    instruction::pointer_t  f_instruction = instruction::pointer_t();
};
...
#define INSTRUCTION(name)   instruction_factory g_instruction_factory_##name(std::make_shared<inst_##name>())
