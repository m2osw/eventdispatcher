...
typedef std::shared_ptr<instruction::map_t> map_pointer_t;

map_pointer_t      g_instructions = map_pointer_t();
...
instruction_factory::instruction_factory(instruction::pointer_t i)
    : f_instruction(i)
{
    if(g_instructions == nullptr)
    {
        g_instructions = std::make_shared<instruction::map_t>();
    }
