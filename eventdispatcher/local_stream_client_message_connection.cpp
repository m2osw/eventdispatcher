...
/** \file
 * \brief The Unix base socket implementing support for messages.
 *
 ...
 */
void local_stream_client_message_connection::process_line(std::string const & line)
{
    if(line.empty())
    {
        return;
    ...
    else
    {
        ...
        //
        SNAP_LOG_ERROR
                << "local_stream_client_message_connection::process_line() was asked to process an invalid message ("
                << line
                << ")"
                << SNAP_LOG_SEND;
    }
